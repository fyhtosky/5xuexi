...
import com.sj.yinjiaoyun.xuexi.activity.PhotoPickerActivity;
import com.sj.yinjiaoyun.xuexi.entry.Photo;
import com.sj.yinjiaoyun.xuexi.utils.ImageLoader;
            ...
            public void onClick(View v) {
                ...
                if(mSelectedPhotos.contains(path)) {
                    ...
                    if(mSelectedPhotos.size() >= mMaxNum) {
                        ToastUtil.showShortToast(mContext,"图片最多只能选择9张");
                        return;
                    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if(getItemViewType(position) == TYPE_CAMERA) {
            convertView = LayoutInflater.from(mContext).inflate(
                    R.layout.item_camera_layout, parent,false);
