...
public class UploadImageActivity extends AppCompatActivity {
    ...
    protected void onResume() {
        super.onResume();
        ToastUtil.getNetworkSatte(UploadImageActivity.this);
    }
    @Override
    ...
    private AsyncTask getPhotosTask = new AsyncTask() {
        ...
        protected void onPreExecute() {
            mProgressDialog = ProgressDialog.show(UploadImageActivity.this, null, "loading...");
        }

        @Override
        protected Object doInBackground(Object[] params) {
            mFolderMap = PhotoUtils.getPhotos(
                    UploadImageActivity.this.getApplicationContext());
            return null;
        }
    ...
    private void getPhotosSuccess() {
        ...
        mGridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if ( position == 0) {
                    //发起请求获得用户许可,可以在此请求多个权限
                    if (ContextCompat.checkSelfPermission(UploadImageActivity.this, android.Manifest.permission.CAMERA)
                            != PackageManager.PERMISSION_GRANTED) {
                        ActivityCompat.requestPermissions(UploadImageActivity.this,new String[] {android.Manifest.permission.CAMERA},
                                OPEN_CAMERA  );
                    }else {
                        //同意该权限则直接启动相机
                        showCamera();
...
//                    cropImg(p.getPath());
                    ...
                    if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.N){
                        inputUri=FileProvider.getUriForFile(UploadImageActivity.this, "com.sj.yinjiaoyun.xuexi.fileProvider", file);
                    }else {
...
//        }
        ...
        if(cameraIntent.resolveActivity(getPackageManager()) != null){
            // 设置系统相机拍照后的输出路径
            if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.N){
                Uri uriForFile = FileProvider.getUriForFile(UploadImageActivity.this, "com.sj.yinjiaoyun.xuexi.fileProvider", mTmpFile);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode == REQUEST_CAMERA){
            if(resultCode == Activity.RESULT_OK) {
                if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.N){
                    Uri inputUri=FileProvider.getUriForFile(UploadImageActivity.this, "com.sj.yinjiaoyun.xuexi.fileProvider", mTmpFile);
                    startPhotoZoom(inputUri,mTmpFile);//设置输入类型
                }else {
                ...
                "image=" + photo + "&fileName=" + new Date().getTime() + ".png" + "&mimeType=png&groupType="+String.valueOf(1), new CallBack<StringBuffer>() {
                    ...
                    public void onSuccess(StringBuffer result) {
                        ...
                        if(bean.isSuccess()){
                            ...
                            //把返回数据存入Intent
                            intent.putExtra("result", bean.getData().getUrl());
                            //设置返回数据
                            UploadImageActivity.this.setResult(Activity.RESULT_OK, intent);
                            //关闭Activity
                            UploadImageActivity.this.finish();
                        }else{
                            ToastUtil.showShortToast(UploadImageActivity.this,bean.getMessage());
                        }
                    }
